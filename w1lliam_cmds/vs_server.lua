...
RegisterCommand('w_frystid', function(source, args)
    if source ~= 0 then
...
RegisterCommand('w_frysvejr', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
...
RegisterCommand('w_vejr', function(source, args)
    if source == 0 then
...
RegisterCommand('w_blackout', function(source)
    if source == 0 then
...
RegisterCommand('w_morgen', function(source)
    if source == 0 then
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(9)
        TriggerClientEvent('vSync:notify', source, 'Tid sat til ~y~morgen~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('w_dag', function(source)
    if source == 0 then
        print("For console, use the \"/time <hh> <mm>\" command instead!")
        return
    end
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(12)
        TriggerClientEvent('vSync:notify', source, 'Tid sat til ~y~dag~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)

RegisterCommand('w_aften', function(source)
    if source == 0 then
        print("For console, use the \"/time <hh> <mm>\" command instead!")
        return
    end
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(18)
        TriggerClientEvent('vSync:notify', source, 'Tid sat til ~y~aften~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('w_nat', function(source)
    if source == 0 then
        print("For console, use the \"/time <hh> <mm>\" command instead!")
        return
    end
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(23)
        TriggerClientEvent('vSync:notify', source, 'Tid sat til ~y~nat~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
...
RegisterCommand('w_tid', function(source, args, rawCommand)
    if source == 0 then
